...
import { Artifact, Session, ComponentVariation, SavedProject } from './types';
...
export default function App() {
  ...
  const [focusedArtifactIndex, setFocusedArtifactIndex] = useState<
    number | null
  >(null);
      ...
      try {
        ...
        if (jsonMatch) {
          ...
          if (Array.isArray(newPlaceholders) && newPlaceholders.length > 0) {
            const shuffled = newPlaceholders
  ...
  const saveProject = async (session: Session) => {
    setIsSaving(true);
    try {
      const response = await fetch('/api/projects', {
        method: 'POST',
  ...
  // Load a saved project into view
  const loadProjectIntoView = (project: SavedProject) => {
  ...
  const downloadArtifact = (artifact: Artifact) => {
    ...
    a.href = url;
    a.download = `${artifact.styleName.replace(/\s+/g, '-').toLowerCase()}.html`;
  ...
  const downloadProjectAsZip = async (session: Session) => {
    const JSZip = (await import('jszip')).default;
    const zip = new JSZip();

    // Add each artifact
    session.artifacts.forEach((artifact, index) => {
      const filename = `${index + 1}-${artifact.styleName.replace(/\s+/g, '-').toLowerCase()}.html`;
      zip.file(filename, artifact.html);
    });

    // Create an index.html that links to all artifacts
    const indexHtml = `<!DOCTYPE html>
<html lang="en">
<head>
  <meta charset="UTF-8">
  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  <title>${session.prompt} - DLM UI</title>
  <style>
...
<body>
  ...
  <p>Generated by DLM UI</p>
  <div class="grid">
    ${session.artifacts
      .map(
        (artifact, index) => `
    <div class="card">
      <iframe src="${index + 1}-${artifact.styleName.replace(/\s+/g, '-').toLowerCase()}.html"></iframe>
      <div class="card-footer">
        <a href="${index + 1}-${artifact.styleName.replace(/\s+/g, '-').toLowerCase()}.html" target="_blank">${artifact.styleName}</a>
      </div>
    </div>`
      )
      .join('')}
    ...
    const a = document.createElement('a');
    a.href = url;
    a.download = `${session.prompt.slice(0, 30).replace(/\s+/g, '-').toLowerCase()}-dlm-ui.zip`;
    document.body.appendChild(a);
    a.click();
    document.body.removeChild(a);
    URL.revokeObjectURL(url);
          ...
          ? {
              ...
              artifacts: sess.artifacts.map((art, j) =>
                j === focusedArtifactIndex
                  ? { ...art, html, status: 'complete' as const }
                  : art
              ),
            }
          : sess
...
Return ONLY a raw JSON array of 3 *NEW*, creative names for these directions (e.g. ["Tactile Risograph Press", "Kinetic Silhouette Balance", "Primary Pigment Gridwork"]).
        ...
        const generateArtifact = async (
          ...
          try {
            const prompt = `
You are DLM UI. Create a stunning, high-fidelity UI component for: "${trimmedInput}".
...
Return ONLY RAW HTML. No markdown fences.
          `.trim();
            ...
            if (finalHtml.endsWith('```'))
              finalHtml = finalHtml.substring(0, finalHtml.length - 3).trimEnd();
        ...
        // Get the final session state for saving
        setSessions((prev) => {
          const finalSession = prev.find((s) => s.id === sessionId);
          if (finalSession) {
            // Save in background
            saveProject(finalSession);
          }
          return prev;
        });
      } catch (e) {
  ...
  if (hasStarted) {
    if (focusedArtifactIndex !== null) {
      ...
      canGoForward =
        focusedArtifactIndex < (currentSession?.artifacts.length || 0) - 1;
    } else {
  ...
  return (
    <>
          ...
          <div className="projects-list">
            {projectsLoading ? (
              ...
              <div className="empty-projects">
                ...
                <span>Generate some UI components to get started!</span>
              </div>
            ) : (
              savedProjects.map((project) => (
                <div key={project.id} className="project-card">
                  <div className="project-card-header">
                    <span className="project-prompt">{project.prompt}</span>
                    <span className="project-date">
                      {new Date(project.savedAt || project.timestamp).toLocaleDateString()}
                    </span>
                  </div>
                  <div className="project-artifacts-preview">
                    {project.artifacts.slice(0, 3).map((artifact) => (
                      <div key={artifact.id} className="project-artifact-thumb">
                        <iframe
                  ...
                  <div className="project-actions">
                    <button
                      ...
                      onClick={() => loadProjectIntoView(project)}
                    >
                      <GridIcon /> Open
                    </button>
                    <button
      ...
      <div className="immersive-app">
        ...
        <div
          className={`stage-container ${focusedArtifactIndex !== null ? 'mode-focus' : 'mode-split'}`}
        >
        ...
        >
          ...
          <div className="action-buttons">
            ...
            {currentSession && focusedArtifactIndex !== null && (
              <button
                onClick={() =>
                  downloadArtifact(currentSession.artifacts[focusedArtifactIndex])
                }
              >
                <DownloadIcon /> Download
              </button>
            ...
            {currentSession && (
              <button
                onClick={() => saveProject(currentSession)}
                disabled={isSaving}
              >
